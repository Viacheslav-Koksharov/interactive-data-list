...
const TableList = () => {
  ...
  const selectedItemlength = itemsList.reduce(
    (selected, item) => selected + item.isSelect,
    0,
  );
  ...
  const deleteItem = e => {
    e.stopPropagation();
    setItemsList(
      itemsList.filter(item => item.uuid !== Number(e.target.closest('tr').id)),
    );
  };
